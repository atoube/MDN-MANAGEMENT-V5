...
import { Space, Folder, Document } from '../types/documentation';
...
export default function Documents() {
  ...
  // Documents filtrés par espace et dossier (exclure les supprimés)
  ...
  const stats = useMemo(() => {
    ...
    const totalViews = activeDocuments.reduce((sum, doc) => sum + doc.views_count, 0);
  ...
  const handleCreateSpace = async (spaceData: Omit<Space, 'id' | 'created_at' | 'updated_at'>) => {
    try {
  ...
  const handleCreateDocument = async (documentData: Omit<Document, 'id' | 'created_at' | 'updated_at' | 'version' | 'views_count'>) => {
    try {

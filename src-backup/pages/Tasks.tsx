...
import { CommentManager } from '../components/tasks/CommentManager';
import { MetricsDashboard } from '../components/tasks/MetricsDashboard';
...
import { GamificationDashboard } from '../components/tasks/GamificationDashboard';
import { IntegrationManager } from '../components/tasks/IntegrationManager';
import { TimeDashboard } from '../components/tasks/TimeDashboard';
...
export default function Tasks() {
  const { user } = useAuth();
  const [view, setView] = useState<'kanban' | 'scrum' | 'archives' | 'time' | 'tags' | 'comments' | 'metrics' | 'templates' | 'workflows' | 'gamification' | 'integrations'>('kanban');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            ...
            {view === 'time' && 'Dashboard Temps - Suivi du temps et productivité'}
            {view === 'tags' && 'Gestion des Tags - Organisation et catégorisation'}
            {view === 'comments' && 'Gestion des Commentaires - Discussions et collaborations'}
            {view === 'metrics' && 'Dashboard Métriques - Analyse de la productivité et performances'}
            {view === 'templates' && 'Gestion des Templates - Modèles de tâches réutilisables'}
            {view === 'workflows' && 'Gestion des Workflows - Automatisation des processus'}
            {view === 'gamification' && 'Gamification - Points, badges et classements pour motiver l\'équipe'}
            {view === 'integrations' && 'Intégrations - Connecter le système avec des services externes'}
          </p>
        </div>
            ...
            <>
              ...
              <Button
                onClick={() => {
                  ...
                  setIsEditMode(true); // Mode édition pour créer une nouvelle tâche
                  setIsTaskModalOpen(true);
                }}
              >
      ...
      <Tabs value={view} onValueChange={(value) => setView(value as 'kanban' | 'scrum' | 'archives' | 'time' | 'tags' | 'comments' | 'metrics' | 'templates' | 'workflows' | 'gamification' | 'integrations')}>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="pt-2 border-t">
                              <TagSelector
                                taskId={task.id}
                                selectedTags={getTaskTags(task.id)}
                                onTagsChange={() => {}}
                                compact
                              />
                            </div>
                            ...
                            {task.status === 'review' && (user?.role === 'admin' || user?.role === 'hr') && (
      ...
      {isTaskModalOpen && (
        <TaskModal
          ...
          columns={columns}
          sprints={[]} // TODO: Ajouter les sprints si nécessaire
          currentSprint={null} // TODO: Ajouter le sprint actuel si nécessaire
          view={view === 'kanban' || view === 'scrum' || view === 'archives' ? view : 'kanban'}
          isEditMode={isEditMode}
          onCreateTask={(data) => {
            console.log('Création de tâche avec données:', data);
          ...
          onUpdateTask={(id, data) => {
            updateTask.mutate({ id, updates: data });
            setIsTaskModalOpen(false);
            setSelectedTask(null);
            setIsEditMode(false);
          }}
         ...
         <TemplateSelector
           onTemplateSelect={(template) => {
             ...
             setSelectedTask({
               ...
               assigned_to: user?.id?.toString() || '1',
               due_date: '',
               ...
               story_points: template.estimatedHours
             });
             setIsEditMode(false);
             setIsTaskModalOpen(true);
           }}
